...
void transformPoint()
{
	for (int i = 0; i < 8; i++)
	{
		point[i] = projection * camera * key_rotate * ctm * point[i];
		point[i] /= point[i].w;
	}
}
...
{
	float frustum[6];
	fscanf(input,"%f", &frustum[0]);
	fscanf(input,"%f", &frustum[1]);
	fscanf(input,"%f", &frustum[2]);
	fscanf(input,"%f", &frustum[3]);
	fscanf(input,"%f", &frustum[4]);
	fscanf(input,"%f", &frustum[5]);
	projection = Frustum(frustum[0], frustum[1], frustum[2], frustum[3], frustum[4], frustum[5]);
}

void setOrtho()
{
	float ortho[6];
	fscanf(input,"%f", &ortho[0]);
	fscanf(input,"%f", &ortho[1]);
	fscanf(input,"%f", &ortho[2]);
	fscanf(input,"%f", &ortho[3]);
	fscanf(input,"%f", &ortho[4]);
	fscanf(input,"%f", &ortho[5]);
	projection = Ortho(ortho[0], ortho[1], ortho[2], ortho[3], ortho[4], ortho[5]);
}
...
void setLookat()
{
	...
	vec4 at;
	vec4 up;
	fscanf(input,"%f", &eye.x);
	fscanf(input,"%f", &eye.y);
	fscanf(input,"%f", &eye.z);
	fscanf(input,"%f", &at.x);
	fscanf(input,"%f", &at.y);
	fscanf(input,"%f", &at.z);
	fscanf(input,"%f", &up.x);
...
void readData()
{
	...
	{
		fscanf(input,"%s", text);
		if (strcmp (text, "FRUSTUM") == 0)
		{
			setFrustum();
		}
		else if (strcmp (text, "ORTHO") == 0)
		{
			setOrtho();
		}
		else if (strcmp (text, "LOOKAT") == 0)
		{
			setLookat();
		}
		else if (strcmp (text, "SOLID_CUBE") == 0)
		{
			setUnitCube();
			transformPoint();
			fillCube();
		...
		else if (strcmp (text, "LOAD_IDENTITY_MATRIX") == 0)
		{
			resetCTM();
		}
