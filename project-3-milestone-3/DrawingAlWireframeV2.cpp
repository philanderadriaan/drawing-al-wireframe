...
void transform(int group, int rx, int ry, int rz, float mx, float my, float mz)
{
	group_transformations[group] = Translate(group_inverses[group]) * Translate(mx, my, mz) * RotateX(rx) * RotateY(ry) * RotateZ(rz) * Translate(group_origins[group]) * group_transformations[group];
	...
	if (group == TORSO)
	{
		group_origins[TORSO] = group_transformations[TORSO] * group_origins[TORSO];
		group_inverses[TORSO].x = -group_origins[TORSO].x;
		group_inverses[TORSO].y = -group_origins[TORSO].y;
		group_inverses[TORSO].z = -group_origins[TORSO].z;
	}
}
...
void myspecialkey(int key, int mousex, int mousey)
{
	...
	{
		transform(moving_group, xRot, 0, 0, 0, 0, 0);
	}
	glutPostRedisplay();
}
...
void swimSetup()
{
	...
	group_transformations[10] = Scale(0.5, 0.5, 0.5) * group_transformations[10];
	transform(TORSO, 0, 90, 0, radius, 0, 0);
	transform(LEFT_UPPER_ARM, -240, 0, 0, 0, 0, 0);
	transform(RIGHT_UPPER_ARM, -60, 0, 0, 0, 0, 0);
	transform(LEFT_FOOT, 45, 0, 0, 0, 0, 0);
	transform(RIGHT_FOOT, 45, 0, 0, 0, 0, 0);
}


void swimIdle(int idle_count)
{
	int sequence = idle_count % 12;
	transform(TORSO, 0, 0, 10, 0, 0, 0);
	transform(LEFT_UPPER_ARM, 30, 0, 0, 0, 0, 0);
	transform(RIGHT_UPPER_ARM, 30, 0, 0, 0, 0, 0);
	if (sequence == 0 || sequence == 9)
	{
		transform(LEFT_LOWER_ARM, 0, -60, 0, 0, 0, 0);
		transform(RIGHT_LOWER_ARM, 0, -60, 0, 0, 0, 0);
		transform(LEFT_UPPER_LEG, -10, 0, 0, 0, 0, 0);
		transform(RIGHT_UPPER_LEG, 10, 0, 0, 0, 0, 0);
		transform(LEFT_LOWER_LEG, -10, 0, 0, 0, 0, 0);
		transform(RIGHT_LOWER_LEG, 10, 0, 0, 0, 0, 0);
		transform(LEFT_FOOT, -30, 0, 0, 0, 0, 0);
		transform(RIGHT_FOOT, 30, 0, 0, 0, 0, 0);
	}
	else if (sequence == 3 || sequence == 6)
	{
		transform(LEFT_LOWER_ARM, 0, 60, 0, 0, 0, 0);
		transform(RIGHT_LOWER_ARM, 0, 60, 0, 0, 0, 0);
		transform(LEFT_UPPER_LEG, 10, 0, 0, 0, 0, 0);
	...
	idle_count++;
	glutPostRedisplay();
	glutTimerFunc(41, swimIdle, idle_count);
}
...
void diveSetup()
{
	...
	transform(LEFT_LOWER_ARM, 0, 60, 0, 0, 0, 0);
	transform(RIGHT_LOWER_ARM, 0, -60, 0, 0, 0, 0);
	transform(TORSO, 0, -90, 0, 0, 0, 0);
}

void diveIdle(int idle_count)
{
	int sequence = idle_count % 12;
	if (idle_count == 0)
	{
	...
	{
		//lower arms
		transform(LEFT_UPPER_ARM, 0, 0, -5, 0, 0, 0);
		transform(RIGHT_UPPER_ARM, 0, 0, 5, 0, 0, 0);
	}
	else if (idle_count > 10 && idle_count < 20)
	{
		//raise arm
		transform(LEFT_UPPER_ARM, -20, 0, 0, 0, 0, 0);
		transform(RIGHT_UPPER_ARM, -20, 0, 0, 0, 0, 0);
	}
	else if (idle_count == 20)
	{
		//lean back
		transform(LEFT_UPPER_LEG, 20, 0, 0, 0, 0, 0);
		transform(RIGHT_UPPER_LEG, 20, 0, 0, 0, 0, 0);
		transform(LEFT_LOWER_LEG, 20, 0, 0, 0, 0, 0);
		transform(RIGHT_LOWER_LEG, 20, 0, 0, 0, 0, 0);
		transform(LEFT_FOOT, 45, 0, 0, 0, 0, 0);
		transform(RIGHT_FOOT, 45, 0, 0, 0, 0, 0);
	}
	else if (idle_count > 20 && idle_count < 30)
	{
		//backflip
		transform(LEFT_UPPER_LEG, -1, 0, 0, 0, 0, 0);
		transform(RIGHT_UPPER_LEG, -1, 0, 0, 0, 0, 0);
		transform(LEFT_LOWER_LEG, -1, 0, 0, 0, 0, 0);
		transform(RIGHT_LOWER_LEG, -1, 0, 0, 0, 0, 0);
		transform(TORSO, 0, 0, -20, 0, 0, 0);
	}
	else if (idle_count == 30)
	{
		//straighten
		transform(LEFT_UPPER_LEG, -20, 0, 0, 0, 0, 0);
		transform(RIGHT_UPPER_LEG, -20, 0, 0, 0, 0, 0);
	...
	else if (idle_count > 30 && idle_count < 40)
	{
		//spin
		transform(TORSO, 0, -20, 0, 0, 0, 0);
	}
	else if (idle_count > 40 && idle_count < 50)
	{
		//curl up
		transform(LEFT_UPPER_ARM, 10, 0, 0, 0, 0, 0);
		transform(RIGHT_UPPER_ARM, 10, 0, 0, 0, 0, 0);
		transform(LEFT_UPPER_LEG, -15, 0, 0, 0, 0, 0);
		transform(RIGHT_UPPER_LEG, -15, 0, 0, 0, 0, 0);
		transform(TORSO, 0, 0, -20, 0, 0, 0);
	}
	else if (idle_count > 50 && idle_count < 80)
	{
		//spin
		transform(TORSO, 0, 0, -19, 0, 0, 0);
	}
	else if (idle_count > 80 && idle_count < 90)
	{
		//straighten
		transform(LEFT_UPPER_ARM, -10, 0, 0, 0, 0, 0);
		transform(RIGHT_UPPER_ARM, -10, 0, 0, 0, 0, 0);
	...
	else if (idle_count > 100 && idle_count < 110)
	{
		//turn around
		transform(TORSO, 0, 0, 0, 0, -1, 0);
	}
	else if (idle_count == 110)
	{
		//swim up
		transform(TORSO, 0, 0, 180, 0, -20, 0);
		transform(LEFT_UPPER_ARM, 0, 0, 180, 0, 0, 0);
	}
	else if (idle_count > 110)
	{
		transform(TORSO, 0, 0, 0, 0, 1, 0);
		transform(LEFT_UPPER_ARM, 60, 0, 0, 0, 0, 0);
		transform(RIGHT_UPPER_ARM, 60, 0, 0, 0, 0, 0);
		if (sequence == 0 || sequence == 5 || sequence == 6 || sequence == 11)
		{
			transform(LEFT_UPPER_LEG, 10, 0, 0, 0, 0, 0);
			transform(RIGHT_UPPER_LEG, -10, 0, 0, 0, 0, 0);
			transform(LEFT_LOWER_LEG, 10, 0, 0, 0, 0, 0);
		...
		else if (sequence == 2 || sequence == 4 || sequence == 8 || sequence == 10)
		{
			transform(LEFT_UPPER_LEG, -10, 0, 0, 0, 0, 0);
			transform(RIGHT_UPPER_LEG, 10, 0, 0, 0, 0, 0);
			transform(LEFT_LOWER_LEG, -10, 0, 0, 0, 0, 0);
	...
	idle_count++;
	glutPostRedisplay();
	glutTimerFunc(80, diveIdle, idle_count);
}
...
int main(int argc, char** argv)
{
	...
	group_dependencies[LEFT_UPPER_LEG] = TORSO;
	group_dependencies[RIGHT_UPPER_LEG] = TORSO;
	group_dependencies[LEFT_LOWER_ARM] = LEFT_UPPER_ARM;
	group_dependencies[RIGHT_LOWER_ARM] = RIGHT_UPPER_ARM;
	group_dependencies[LEFT_UPPER_ARM] = TORSO;
	group_dependencies[RIGHT_UPPER_ARM] = TORSO;
	...
	printf("Enter the action of Al (no blanks): \n(1) Swimming\n(2) Olympic Diving\n(3) User Controlled\n");
	char action_input[50];
	gets(action_input);
	int action = atoi(action_input);
	glutInit(&argc, argv);
	...
	{
		swimSetup();
		glutTimerFunc(41, swimIdle, 0);
	}
	else if (action == 2)
	{
		diveSetup();
		glutTimerFunc(80, diveIdle, 0);
	}
